...
@section('content')
    <div class="container py-4" data-aos="fade-up" data-aos-delay="100">
        ...
        <div class="card shadow-sm rounded-3 border-0 mb-4">
                        ...
                        <div class="d-flex justify-content-md-end">
                            <div class="dropdown me-2">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="statusFilter">
                                    <li><a class="dropdown-item status-option" data-value="Semua" href="#">Semua</a></li>
                                    <li><a class="dropdown-item status-option" data-value="Menunggu" href="#">Menunggu</a></li>
                                    <li><a class="dropdown-item status-option" data-value="Diproses" href="#">Diproses</a></li>
                                    <li><a class="dropdown-item status-option" data-value="Selesai" href="#">Selesai</a></li>
                                </ul>
                            </div>
                            <div class="dropdown">
                                <button class="btn btn-outline-secondary dropdown-toggle" type="button" id="sortOrder"
                                    data-bs-toggle="dropdown" aria-expanded="false">
                                    <i class="fas fa-sort me-1"></i> Urutkan
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="sortOrder">
                                    <li><a class="dropdown-item sort-option" data-value="desc" href="#">Terbaru</a></li>
                                    <li><a class="dropdown-item sort-option" data-value="asc" href="#">Terlama</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody id="pengaduanList">
                        @forelse($pengaduans as $index => $pengaduan)
                            <tr data-status="{{ $pengaduan->status }}" data-date="{{ $pengaduan->created_at }}">
                                <td class="px-4">{{ $pengaduans->firstItem() + $index }}</td>
                                <td>
                                    <div class="fw-bold text-primary">{{ $pengaduan->judul }}</div>
                                    <small
                                        class="text-muted">{{ \Carbon\Carbon::parse($pengaduan->created_at)->format('d M Y') }}</small>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <button class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                            data-bs-target="#detailModal{{ $pengaduan->id }}" title="Lihat Detail">
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-5">
                                    <div class="py-4">
                                        ...
                                        <p class="text-muted">Belum ada pengaduan yang tersedia atau sesuai dengan pencarian
                                            Anda</p>
                                    </div>
                                </td>
                            </tr>
        ...
        @foreach ($pengaduans as $pengaduan)
            <div class="modal fade " id="detailModal{{ $pengaduan->id }}" tabindex="-1"
                aria-labelledby="modalLabel{{ $pengaduan->id }}" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-scrollable">
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-4">
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div>{{ \Carbon\Carbon::parse($pengaduan->created_at)->format('d M Y, H:i') }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                            ...
                            <div class="card mb-4">
                                <div class="card-header bg-light">
                                    <h6 class="mb-0"><i class="fas fa-images me-2"></i>Lampiran</h6>
                                </div>
                                <div class="card-body">
                                    <a href="{{ asset('storage/' . $pengaduan->lampiran) }}" target="_blank"><img
                                            src="{{ asset('storage/' . $pengaduan->lampiran) }}" alt=""></a>
                                </div>
                            </div>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const rows = Array.from(document.querySelectorAll('#pengaduanList tr'));

        function applyFilters() {
            // Filter
            rows.forEach(row => {
                const status = row.getAttribute('data-status');
                const show = currentStatus === 'Semua' || status === currentStatus;
                row.style.display = show ? '' : 'none';
            });

            // Sort
            const tbody = document.querySelector('#pengaduanList');
